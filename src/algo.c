...
{
	b->samples = fftwf_alloc_real(2 * b->sample_count);
	b->samples_sc = fftwf_alloc_real(2 * b->sample_count);
	b->waveform = fftwf_alloc_real(2 * b->sample_rate);
	b->waveform_sc = fftwf_alloc_real(2 * b->sample_rate);
	b->fft = fftwf_alloc_complex(b->sample_count + 1);
	b->sc_fft = fftwf_alloc_complex(b->sample_count + 1);
	b->tic_wf = fftwf_alloc_real(b->sample_rate);
	b->slice_wf = fftwf_alloc_real(b->sample_rate);
	b->tic_fft = fftwf_alloc_complex(b->sample_rate/2 + 1);
	b->slice_fft = fftwf_alloc_complex(b->sample_rate/2 + 1);
	b->tic_c = fftwf_alloc_real(2 * b->sample_count);
...
static void locate_events(struct processing_buffers *p)
{
	...
	for(i=0, j=0; i < 2*count; i++) {
		if(events[i].pos == (uint64_t)-1 ||
		   events[i].pos + p->timestamp < (unsigned)p->sample_count ||
